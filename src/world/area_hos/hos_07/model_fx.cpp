...
s32 GlowingStarAlpha = 255;
s32 GivingPowerAlpha = 255;
...
void setup_gfx_glowing_star(void) {
    gDPSetCombineMode(gMainGfxPos++, PM_CC_05, PM_CC_05);
    gDPSetPrimColor(gMainGfxPos++, 0, 0, 0, 0, 0, GlowingStarAlpha);
}

API_CALLABLE(SetGlowingStarAlpha) {
    Bytecode* args = script->ptrReadPos;

    GlowingStarAlpha = evt_get_variable(script, *args++);
    return ApiStatus_DONE2;
}

void setup_gfx_giving_power(void) {
    gDPSetCombineMode(gMainGfxPos++, PM_CC_05, PM_CC_05);
    gDPSetPrimColor(gMainGfxPos++, 0, 0, 0, 0, 0, GivingPowerAlpha);
}

API_CALLABLE(SetGivingPowerAlpha) {
    Bytecode* args = script->ptrReadPos;

    GivingPowerAlpha = evt_get_variable(script, *args++);
    return ApiStatus_DONE2;
}

EvtScript EVS_UpdateGivingPower = {
    Loop(0)
        Call(MakeLerp, 180, 80, 45, EASING_COS_IN_OUT)
        Loop(0)
            Call(UpdateLerp)
            Call(SetGivingPowerAlpha, LVar0)
            Wait(1)
            IfEq(LVar1, 0)
                BreakLoop
            EndIf
...
EvtScript EVS_SetupModelFX = {
    ...
    Else
        ...
        Set(LVar0, 255)
        Call(SetStarBeamMasterAlpha, MV_StarBeamFXPtr, LVar0)
        Call(GetEntryID, LVar0)
        IfEq(LVar0, hos_01_ENTRY_1)
            Call(func_80240B10_A11390, MV_StarBeamFXPtr)
        Else
            Call(func_80240AAC_A1132C, MV_StarBeamFXPtr)
        EndIf
    EndIf
    ...
    Call(SetCustomGfxBuilders, CUSTOM_GFX_0, Ref(setup_gfx_glowing_star), nullptr)
    Call(TranslateModel, MODEL_power, 0, 2, 0)
    Call(SetModelCustomGfx, MODEL_power, CUSTOM_GFX_1, ENV_TINT_UNCHANGED)
    Call(SetCustomGfxBuilders, CUSTOM_GFX_1, Ref(setup_gfx_giving_power), nullptr)
    Thread
        Loop(0)
            Call(MakeLerp, 255, 100, 60, EASING_COS_IN_OUT)
            Loop(0)
                Call(UpdateLerp)
                Call(SetGlowingStarAlpha, LVar0)
                Wait(1)
                IfEq(LVar1, 0)
                    BreakLoop
                EndIf
    ...
    Thread
        IfGe(GB_StoryProgress, STORY_CH8_OPENED_PATH_TO_STAR_WAY)
            ExecWait(EVS_UpdateGivingPower)
        Else
            Call(SetGivingPowerAlpha, 0)
            Label(10)
