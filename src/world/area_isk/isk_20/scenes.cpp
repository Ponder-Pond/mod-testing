...
EvtScript EVS_OpenRightGate = {
    ...
    Call(SetCamSpeed, CAM_DEFAULT, 5)
    ...
    Wait(1)
    Call(PlaySound, SOUND_LARGE_GATE_OPEN)
    Call(ModifyColliderFlags, MODIFY_COLLIDER_FLAGS_SET_BITS, COLLIDER_RightGate, COLLIDER_FLAGS_UPPER_MASK)

    Label(10)
    Call(TranslateModel, MODEL_RightGate, 0, LVarD, 0)
    Add(LVarD, 2)
...
EvtScript EVS_CloseRightGate = {
    ...
    Call(ModifyColliderFlags, MODIFY_COLLIDER_FLAGS_CLEAR_BITS, COLLIDER_RightGate, COLLIDER_FLAGS_UPPER_MASK)
    Call(StopSound, SOUND_LARGE_GATE_OPEN)
    Call(PlaySound, SOUND_LARGE_GATE_CLOSE)
    Call(ShakeCam, CAM_BATTLE, 0, 4, Float(0.5))
    Wait(30)
